...
{
  ROS_DEBUG("[YoloObjectDetector] image received.");
  ...
  if (cam_image) {
    ...
    MatWithHeader_ imageAndHeader2 = getIplImageWithHeader();
    ROS_img = imageAndHeader2.image;
    mat_to_image(ROS_img, buff_ + buffIndex_);
    headerBuff_[buffIndex_] = imageAndHeader2.header;
    ...
    ROS_INFO("[YoloObjectDetector] detections=%d", nboxes);
    ...
    if (!publishDetectionImage(cv::Mat(cvImage))) {
      ROS_DEBUG("Detection image has not been broadcast.");
    }
    ...
    ROS_WARN("[YoloObjectDetector] not image");
  }
  return;
}
